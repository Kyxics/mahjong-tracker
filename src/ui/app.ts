...
type Sheet =
  | {
      kind: 'win';
      step: 'winner' | 'how' | 'value' | 'extras' | 'confirm';
...
function openWin(): void {
  sheet = { kind: 'win', step: 'winner', wins: [], cur: { raw: defaultRaw() }, inputs: {} };
  render();
}
...
function buildOutcome(sh: Sheet): HandOutcome<unknown> | null {
  if (sh.kind === 'win') {
    ...
    const wins = [
      ...sh.wins,
      { winner: sh.cur.winner!, winType: sh.cur.winType!, discarder: sh.cur.discarder, value: parsed.value },
    ];
...
function renderWinSheet(sh: Extract<Sheet, { kind: 'win' }>): HTMLElement {
  ...
  if (sh.step === 'winner') {
    ...
    body.push(h('div', { class: 'btn-row' },
      h('button', { text: 'Back', onclick: () => { sh.step = 'value'; render(); } }),
      canAddWinner
        ? h('button', { text: '+ Winner', onclick: () => {
            const parsed = v.parseValue(sh.cur.raw, s.settings);
            if (!parsed.ok) return;
            sh.wins.push({ winner: sh.cur.winner!, winType: 'discard', discarder: sh.cur.discarder, value: parsed.value });
            sh.cur = { raw: defaultRaw() };
            sh.step = 'winner';
            render();
          } })
        : null,
